    ...
    if (containsSearchVal(article, searchVal)) {
      var $article = $(
        '<div class="gb-card-50">\n' +
          ...
          article.description +
          "</p>\n" +
          '                <a class="red-btn btn-medium" target="_blank" href="' +
          article.link +
          '">View article</a>\n' +
          "            </div>\n" +
          "        </div>\n" +
...
function renderTeam(teams, filters, searchVal) {
  ...
  for (var i = 0; i < teams.length; i++) {
    ...
    if (
      containsFilterCriteria(team, filters) &&
      containsSearchVal(team, searchVal)
    ) {
      var $team = $(
        '<li class="card">\n' +
          '                        <img src="' +
          ...
          team.title +
          '\')" class="red-btn btn-medium">Work with us</button>\n' +
          "                        </div>\n" +
          "                        </div>\n" +
          "                    </li>"
      );
...
function renderEvents(events, searchVal, filters) {
  ...
  for (var i = 0; i < events.length; i++) {
    ...
    if (
      containsFilterCriteria(event, filters) &&
      containsSearchVal(event, searchVal)
    ) {
...
function containsFilterCriteria(element, filters) {
  ...
  for (var i = 0; i < monthFilter.length; i++) {
    if (
      element.date &&
      new Date(element.date).getMonth() === Number(monthFilter[i])
    ) {
      containsMonth = true;
...
function containsElement(filter, element, attr) {
  ...
  for (var i = 0; i < filter.length; i++) {
    if (
      element[attr] &&
      element[attr].toLowerCase().trim() === filter[i].trim()
    ) {
      contains = true;
    }
  }
...
function jobContainsSearchVal(element, searchVal) {
  ...
  return (
    company.indexOf(search) !== -1 || containsSearchVal(element, searchVal)
  );
}
