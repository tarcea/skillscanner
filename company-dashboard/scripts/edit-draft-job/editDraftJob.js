...
var jobDraftId = getParameterByName("id"),
  jobId,
  jobImageUrl,
  userEmail,
  companyName,
...
(function initEditting() {
  ...
  if (sessionStorage.getItem("initEditting")) {
    dataSource = "cache";
  }
  var getOptions = {
    source: dataSource,
  ...
  firestore
    .collection("job_offer_drafts")
    .doc(jobDraftId)
    .get(getOptions)
    .then(function (doc) {
      sessionStorage.setItem("initEditting", true);
...
function PublishJobPost() {
  ...
  if (jobImage) {
    updateToStorage(userEmail, jobImage, jobId)
      .then(function (url) {
        addToDatabase(job, url, jobId).then(function () {
          deleteDraftFromDatabase(jobDraftId).then(function () {
            CloseLoadingBox();
    ...
    addToDatabase(job, jobImageUrl, jobId)
      .then(function () {
        deleteDraftFromDatabase(jobDraftId).then(function () {
          CloseLoadingBox();
...
function deleteDraftFromDatabase(jobDraftId) {
  return firestore.collection("job_offer_drafts").doc(jobDraftId).delete();
}
...
function addToDatabase(job, url, jobId) {
  job.image = url;
